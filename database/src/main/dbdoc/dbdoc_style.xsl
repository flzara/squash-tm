...
xmlns:lb="http://www.liquibase.org/xml/ns/dbchangelog">
	...
	<xsl:template match="/">
		...
		<h1>Squash Tm version ${project.version} : Dictionnaire de donnees</h1>
			...
			<!--
				Here we manually set the relative path because interpolating with $ {project.build.directory} yields invalid URI when built on windows
				(because of the double backslashes)
			-->
			<xsl:variable name="liquiFiles" select="collection('../classes/tm/?select=*.xml')"></xsl:variable>
