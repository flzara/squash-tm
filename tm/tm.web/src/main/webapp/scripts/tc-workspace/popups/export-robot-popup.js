...
define(['jquery', 'tree', 'underscore',
	...
	function ($, zetree, _, attrparser, dateutils) {
		...
		$.widget("squash.exportRobotScriptDialog", $.squash.formDialog, {
			...
			open: function () {
				...
				//We want error message if we have not at least a container or a robot test case in our selection
				...
				if (selection.length > 0 && (countObject.tcRobot > 0) || countObject.container > 0) {
					var name = this._createName();
					$('#export-robot-test-case-filename').val(name);
					this.setState('main');
				}
				else {
			...
			_createUrl: function (nodes, type, filename) {
				...
				var url = squashtm.app.contextRoot + 'test-case-browser/content/robotscripts';
			...
			confirm: function () {
				...
				if ((nodes.length > 0)) {
					var filename = $("#export-robot-test-case-filename").val();
		...
		function init() {
			...
			var dialog = $("#export-robot-test-case-dialog").exportRobotScriptDialog({
				tree: zetree.get()
			});
