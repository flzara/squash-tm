		...
		var typeStepView = AbstractStepView.extend({
			...
			changeExampleGraph: function (type) {
				...
				switch (type) {
					...
					case "BAR":
					case "CUMULATIVE":
						fakeData = this.getFakeData1();
						break;
					case "TREND":
					case "COMPARATIVE":
						fakeData = this.getFakeData2();
						break;
			...
			getFakeData1: function () {
				...
				return {
					"name": translator.get('chart.wizard.example.title'),
					"measures": [{
						"label": "",
						"columnPrototype": {
							"label": "REQUIREMENT_ID",
							"specializedEntityType": {"entityType": "REQUIREMENT", "entityRole": null}
						},
					...
					"axes": [{
						...
						"columnPrototype": {
							...
							"specializedEntityType": {"entityType": "REQUIREMENT_VERSION", "entityRole": null},
							"dataType": "DATE"
						},
						"operation": {"name": "BY_MONTH"}
					}],
					"filters": [],
			...
			// Issue 7432, before we loaded the operation with the name in the selected option.
			// Problem? if the the selected options (in the different select) have the same name, you're screwed
			loadOperationWithId: function (colId, axis) {
				var operation = this.findChoosenOperationWithId(colId) || "NONE";
			...
			reloadData: function () {
				...
				_.chain(["y", "x1", "x2"])
					...
					.each(function (val) {
						var selectedOptionId = $("#axis-" + val).children(":selected").attr("id");
						self.loadOperationWithId(selectedOptionId, val);
						self.loadFilter(selectedOptionId, val);
					});
			...
			// Issue 7432, replaced the name of the selected option with it's id, I suppose that we will have the same problem
			// if we have two selected options with the same name
			loadFilter: function (id, axis) {
				var filter = this.findFilterByColumnId(id);
				var text;
				var self = this;
			...
			find: function (name) {
				...
				if (!_.isEmpty(columnId)) {
					result = {
						column: self.findColumnById(columnId),
						operation: self.findChoosenOperationWithId(columnId),
						label: ""
					};
				}
			...
			// Issue 7432, before we used the name instead of the id.
			// Problem? if the the selected options (in the different select) have the same name, you're screwed
			findChoosenOperationWithId: function (id) {
				return _.chain(this.model.get("operations"))
					.find(function (obj) {
						return obj.column.id == id;
					})
					.result("operation")
					.value();
			...
			findFilterByColumnId: function (id) {
