	...
	return Backbone.Model.extend({
		...
		initialize: function (data) {
			...
			if (chartDef !== null) {
				this.set({
					...
					scope: _.map(chartDef.scope, function (val) {
						val.type = val.type.replace("LIBRARY", "LIBRARIE");
						return val;
					}),
		...
		getOperations: function (chartDef) {
			return _.chain(chartDef)
				...
				.flatten()
				.map(function (val) {
					return _(val).pick('column', 'operation');
				})
				.value();
		},
		...
		toJson: function (param) {
			return JSON.stringify({
				...
				scope: _.map(this.get("scope"), function (val) {
					var newVal = _.clone(val);
					newVal.type = val.type.replace("LIBRARIE", "LIBRARY");
					return newVal;
				})
			});
		},
