 ...
 */
define(
	["jquery", "app/BindView", "handlebars", "backbone.validation", "./InfoListOptionPanel", "./InfoListOptionModel",
		"./InfoListOptionCollection", "app/lnf/Forms", "squashtable/squashtable.options", "squash.configmanager",
		"info-list/IconSelectDialog", "squash.translator", "jquery.squash.confirmdialog"],
	function InfoListPanel($, BindView, Handlebars, Validation, InfoListOptionPanel, InfoListOptionModel, InfoListOptionCollection, Forms, SquashTable, confman, IconPicker, messages) {
		...
		function initOptionsTable(view) {
			...
			var colour = SquashTable.renderer($("#colour-cell-tpl").html())(function (data, type, row) {
				return {value: data, name: "option-colour", code: row.code};
			});
			...
			var colDefs = SquashTable.colDefs()
				...
				.colour({targets: 3, render: colour, data: "colour"})
				.radio({targets: 4, render: radio, data: "isDefault"})
				.button({targets: 5, render: remove, data: ""})
				.build();
...
//						}
			...
			events: {
				...
				"change input[name='option-colour']": "onChangeColour",
			...
			onChangeColour: function (event) {
				var tgt = event.currentTarget;
				var value = tgt.value;
				var code = tgt.getAttribute("code");
				this.options.findWhere({"code": String(code)})
					.set("colour", value);
			},
			...
			onClickListOptionIcon: function (event) {
				...
				this.iconPicker = this.iconPicker || new IconPicker({
					el: "#icon-picker-dialog",
					model: {icon: opt.get("iconName")}
				});
