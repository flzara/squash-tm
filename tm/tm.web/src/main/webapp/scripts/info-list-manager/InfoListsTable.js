 ...
 */
define(
	["jquery", "backbone", "underscore", "../squashtable/squashtable.options", "jquery.squash.oneshotdialog",
		"squash.translator", "app/ws/squashtm.notification", "handlebars", "app/util/StringUtil"],
	function ($, Backbone, _, SquashTable, oneshot, messages, notif, Handlebars, StringUtil) {
		...
		var View = Backbone.View.extend({
			...
			initialize: function () {
				...
				this.$el.on("init.dt", function (event) {
					self.onInitTable(event);
				});
				...
				var editLink = SquashTable.renderer($("#name-cell-tpl").html())(function (data, type, row) {
					return {url: self.viewUrl + "/" + row.id, text: StringUtil.unescape(data)};
				});
...
//							ajax: this.viewUrl,
			...
			fixorder: function () {
				...
				self.$el.DataTable().rows().data().each(function (rowdata, index) {
					...
					var row = self.$el.DataTable().row(function (idx, data, node) {
			...
			onClickSelectHandle: function (event) {
				...
				var growSelection = function ($tr) {
					...
					self.$el.DataTable().rows(function (idx, data, node) {
						return _.contains(range, parseInt(data['1']));
			...
			removeSelectedItems: function removeSelectedItems() {
				...
				var hasBound = _.some(rows.data(), function (data) {
					return $.parseJSON(data.bound) === true;
				});
