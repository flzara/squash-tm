...
(function ($) {
	...
	$.jstree.plugin("core", {
		...
		_fn : {
			...
			reopen : function (is_callback, callback, nodestoopen) { //changed
				var _this = this,
					done = true,
					current = [],
					remaining = [];
                if(!nodestoopen) {nodestoopen = this.data.core.to_open; } //changed
				if(!is_callback) { this.data.core.reopen = false; this.data.core.refreshing = true; }
				if(nodestoopen.length) {
					$.each(/*this.data.core.to_open*/ nodestoopen, function (i, val) {
						if(val == "#") { return true; }
                        var curItem = document.getElementById(val.replace("#",""));     //changed, because error if no node loaded with function val()
						if(curItem && jQuery(curItem).is(".jstree-closed"))
                            { current.push(curItem); }
						    else
                            { remaining.push(val);}
					});
					if(current.length) {
						//this.data.core.to_open = remaining;           //changed
						$.each(current, function (i, val) {
                            _this.open_node(val, function () {
                                    _this.reopen(true, callback, remaining);
                                  }, true);
						});
						done = false;
					}
				...
				if(done) {
					...
					this.data.core.reopen = setTimeout(function () { _this.__callback({}, _this); if(callback) { callback.call(); }  }, 50); //changed
