	...
	function (Backbone, renderUtils, PieView, BarView, LineView, CumulativeView, ComparativeView, TrendView, translator, chartUtils) {
		...
		function buildChart(viewID, jsonChart, vueConf) {
			...
			switch (jsonChart.type) {
				...
				case 'BAR' :
					return generateBarChart(viewID, jsonChart, vueConf);
				case 'LINE' :
					return generateLineChart(viewID, jsonChart, vueConf);
				case 'CUMULATIVE' :
					return generateCumulativeChart(viewID, jsonChart, vueConf);
				case 'COMPARATIVE' :
					return generateComparativeChart(viewID, jsonChart, vueConf);
				case 'TREND' :
					return generateTrendChart(viewID, jsonChart, vueConf);
				default :
					throw jsonChart.chartType + " not supported yet";
			}
		}
