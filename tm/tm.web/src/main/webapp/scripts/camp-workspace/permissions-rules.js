       ...
       *********************************************************/
		...
		function CampaignPermissionsRules() {
			...
			this.canDnD = function (movednodes, newparent) {
				...
				// test-suite can dnd only within the same parent. Copying to a different iteration is allowed though
				if (!squashtm.keyEventListener.ctrl && movednodes.is(':test-suite') && !newparent.is(oldparent)) {
					return false;
				}
