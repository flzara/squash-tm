...
<div class="ui-widget-header ui-state-default ui-corner-all fragment-header">
  ...
  <div class="snap-left" style="height: 100%; width:90%" class="small-margin-left">
    <h2>
  ...
  <div class="toolbar-button-panel">
    ...
    <c:if test="${ writable }">
      ...
      <input type="button" class="sq-btn" value='<f:message key="label.Rename" />' id="rename-campaign-button"
             title='<f:message key="dialog.rename-campaign.title" />'/>
    </c:if>
  ...
  <div class="fragment-tabs fragment-body">
    ...
    <div id="tabs-1">
      ...
      <comp:toggle-panel id="campaign-description-panel"
                         title='${labelDescription} <span class="small txt-discreet">[ID = ${ campaign.id }]</span>'
                         open="true">
        <jsp:attribute name="body">
              ...
              <div class="display-table-row">
                <label for="campaign-description" class="display-table-cell"><f:message
                  key="label.Description"/></label>
                <div id="campaign-description" ${descrRichAttributes}>${hu:clean(campaign["description"])}</div>
              </div>

            <div class="display-table-row">
              <label for="campaign-status" class="display-table-cell"><f:message
                key="campaign.status.combo.label"/></label>
              <div>
                <span id="campaign-status-icon" style="vertical-align:middle"
                      class="sq-icon campaign-status-${campaign.status}"> &nbsp &nbsp</span>
                <span id="campaign-status">${ campaignStatusLabel }</span>
              </div>
            </div>

            <div class="display-table-row">
              <label for="campaign-progress-status" class="display-table-cell"><f:message
                key="campaign.progress_status.label"/></label>
...
<c:if test="${hasCUF}">
      ...
      <comp:toggle-panel id="datepicker-panel" titleKey="label.Planning" open="true">
        <jsp:attribute name="panelButtons">
				<c:if test="${writable}">
				<input id="iteration-planning-button" class="sq-btn" type="button" role="button"
               value="${iterationPlanningButton}"/>
				</c:if>
		</jsp:attribute>
        <jsp:attribute name="body">
