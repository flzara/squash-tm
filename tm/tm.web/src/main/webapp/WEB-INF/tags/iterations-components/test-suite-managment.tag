...
<%@ attribute name="iteration" type="java.lang.Object" required="true"
			  description="the iteration object of which we manage the test suites" %>
	...
	<div class="main-div-suites not-displayed">
		<c:if test="${ creatable }">
			<div class="create-suites-section">
				...
				<input id="ts-popup-addButton" onClick="$('#ts-popup-text').focus()" type="button" class="button"
					   value="${createLabel}"/><br/>
		...
		require(["jquery", "iteration-management"], function ($, main) {
			$(function () {
				...
				var initData = [
					...
					{
						id: '${suite.id}',
						name: '${hu:escape(fn:replace(suite.name, "'", "\\'"))}'
