...
<%@ attribute name="linkable" required="true"
			  description=" boolean that says if the concerned test case is viewed by a user who has LINK rights on this entity" %>
<%@ attribute name="includeIndirectlyVerified" required="true"
...
<table id="verified-requirements-table" class="unstyled-table"
	...
	<tr>
		...
		<c:if test="${milestoneConf.milestoneDatesColumnVisible}">
			<th data-def="sortable, map=milestone-dates, tooltip-target=milestone"><f:message
				key="label.Milestones"/></th>
		</c:if>
		<th data-def="sortable, map=reference"><f:message key="label.Reference"/></th>
		<th data-def="sortable, map=name, sClass=verif-req-description, link-if-readable=${requirementVersionsUrl}/{entity-id}/info">
			<f:message key="requirement.name.label"/></th>
...
<div id="remove-verified-requirement-version-dialog" class="popup-dialog not-displayed"
	 title="<f:message key='label.Confirm'/>">
...
<div id="remove-obsolete-verified-requirement-version-dialog" class="popup-dialog not-displayed"
	 title="<f:message key='dialog.obsolete.requirement.version.removal.confirm.title'/>">
...
<c:if test="${includeIndirectlyVerified }">
	<div id="no-selected-direct-requirement-dialog" class="popup-dialog not-displayed"
		...
		<span><f:message
			key="verified-requirements.table.indirectverifiedrequirements.removalattemptsforbidden.label"/></span>
	</div>
</c:if>
