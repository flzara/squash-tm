...
import org.squashtest.tm.web.internal.controller.testcase.requirement.VerifiedRequirementsDataTableModelHelper.TestCaseVerifiedRequirementsDataTableModelHelper;
import org.squashtest.tm.web.internal.controller.testcase.requirement.VerifiedRequirementsDataTableModelHelper.TestCaseWithCalledStepsVerifiedRequirementsDataTableModelHelper;
import org.squashtest.tm.web.internal.controller.testcase.requirement.VerifiedRequirementsDataTableModelHelper.TestStepVerifiedRequirementsDataTableModelHelper;
...
public class VerifiedRequirementsManagerController {
	...
	public String showTestCaseManager(@PathVariable long testCaseId, Model model,
		...
		List<Long> projectIds = requirementLibraryFinder.findLinkableRequirementLibraries().stream()
			.map(RequirementLibrary::getProject)
			.map(GenericProject::getId)
			.collect(Collectors.toList());
		Optional<Long> activeMilestoneId = activeMilestoneHolder.getActiveMilestoneId();
		Collection<JsTreeNode> linkableLibrariesModel = requirementWorkspaceDisplayService.
			findAllLibraries(projectIds, currentUser, expansionCandidates, activeMilestoneId.get());
