...
def processScripted( hasReq,  ypos2,  statuses2,  importances2, File f2){
	...
	for(int j = 0; j<statuses2.size(); j++){
		ypos2 += widthOfOneIcon
		for(int i = 0; i<importances2.size(); i++){
			println('process combination : kind=scripted and status='+statuses2[j]+' importance='+importances2[i]+' req='+hasReq)
			f2.append('li[rel="test-case"][kind="scripted"][isreqcovered="'+hasReq+'"][importance="'+importances2[i]+'"][status="'+statuses2[j]+'"]> a > .jstree-icon {\n')
			printBackgroundProperty(i*heightOfOneIcon, ypos2, f2)
			f2.append('}\n')
		}
	}
