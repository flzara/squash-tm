 ...
 *
 */
class TestCaseInstructionTest extends Specification {
	TestCaseTarget target = Mock()
	TestCase testCase = Mock()
	TestCaseInstruction instruction = new TestCaseInstruction(target, testCase)
	Facility f = Mock()
	...
	def "should delete test case"() {
		...
		then:
		1 * f.deleteTestCase(target) >> new LogTrain()
	}
